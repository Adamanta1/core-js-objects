...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    const value = Object.entries(objects[i]);
    for (let k = 0; k < value.length; k += 1) {
      const [value1, value2] = [value[k][0], value[k][1]];
      if (newObj[value1]) {
        newObj[value1] += value2;
      } else {
        newObj[value1] = value2;
      }
    }
...
function group(array, keySelector, valueSelector) {
  ...
  let num = 0;
  let key = keySelector(array[num]);
  function test() {
    key = keySelector(array[num]);
    if (map.has(key)) {
      map.get(key).push(valueSelector(array[num]));
    } else {
      map.set(key, [valueSelector(array[num])]);
    }
    if (num < count) {
      num += 1;
      test();
